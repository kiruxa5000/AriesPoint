...
//        $time = $this->value('Tgr');
    ...
    public $taskSQL=[
        ...
        'get_ho'=>['task'=>'%getCorrections&&OS+dhi+dhd=hv+dhp+dhbt=hdop+dhz=ho',
...
//        $this->taskSQL['OMS']['consts'] = '["Ao_1", "Ao_2","Ao_3", "Ao_4", "ho_1", "ho_2","ho_3", "ho_4"]';
        ...
        for($i=1;$i<=$SQ;$i++){
            if(isset($_REQUEST['Ao_'.$i])){
                $this->set('Ao_'.$i, $_REQUEST['Ao_'.$i]);
            }else{
                $this->set('Ao_'.$i, $As[$i-1]+rand(-10,10));
            }

            if(isset($_REQUEST['n_'.$i.'M'])){
                $this->set('n_'.$i, $_REQUEST['n_'.$i.'M']);
            }else{
...
//        $show.="<script>alert('".json_encode($this->SIG)."')</script>";
        ...
        foreach ($consts as $const){
            $newArray[$const]=$this->value($const);
        }
        return ['show'=>$constTable.$show, 'check'=>$check, 'title'=>$taskTitle,'constsTable'=>$constTable,'constsValues'=>$newArray];
    }
...
//        $this->correction = $cor;
        ...
        $this->set('Tgr_1',$this->value('Tgr')+rand(360,1500));
